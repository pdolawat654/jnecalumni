...
<body>
    ...
    $re=mysqli_query($conn,$s);
      ...
      { $n=$ro['name'];}}
    $s="select * from inf where name='$search' and aid='$a'";
    $re=mysqli_query($conn,$s);
      if(mysqli_num_rows($re)>0){
    ...
    <div class="Profile">
      ...
      <table>
        ...
        <tr><?php echo '<img src="data:image/jpeg;base64,'.base64_encode( $ro['image'] ).'" style="height:250px;width:250px;border-radius:125px;"/>';
        }
  }
  else{
    echo "0 results";
  ...
  ?></div></tr>
    ...
    <form action="mess.php" method="POST"><?php  echo '<input type="hidden" name="ai" value="'.
     htmlentities($i).'">'; 
     $_SESSION['e2']=htmlentities($i); ?>
         
      </table>
 <input style="height: 40px;width: 180px;background-color: brown;color: white;margin-bottom: 10px;" type="submit" value="MESSAGE">
      </form>
